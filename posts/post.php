...
<?php
    ...
    $cmt_query = $conn->query("SELECT * FROM comments WHERE post_id = '$post_id'");
    $cmt_query->execute();
    $all_cmt = $cmt_query->fetchAll(PDO::FETCH_OBJ);
    if(isset($_POST['submit']) && isset($_GET['post_id'])){
        $post_id = $_GET['post_id'];
        $insert_cmt = $conn->prepare("INSERT INTO comments(comment, post_id, user_name) VALUES(:comment, :post_id, :user_name)");
        $insert_cmt->execute([
